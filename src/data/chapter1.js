  ...
  topics: [
    {
      ...
      explanation: 'Historically, programming was dominated by the procedural paradigm (like C, Pascal). In procedural programming, the focus is on functions—processing data rather than the data itself. A program is divided into functions, and data is often made global so multiple functions can access it.\n\nThe Problem with Structured Programming:\n1. Unrestricted Access: When data is global, any function can modify it accidentally, leading to bugs that are incredibly difficult to trace in large programs.\n2. Poor Modeling of the Real World: Procedural languages separate data and the functions that operate on them. In the real world, entities (like a car or an employee) consist of both attributes (data) and behaviors (functions) bundled together.\n3. Scalability: As programs grow larger, procedural code becomes a tangled web (often called "spaghetti code") making it hard to maintain or extend.\n\nObject-Oriented Programming (OOP) solves this by bundling data and functions into a single unit called an "object". The data is hidden within the object, and can only be accessed by the object\'s own functions, a concept known as encapsulation.',
      examples: [
        {
    ...
    },
    {
      ...
      explanation: 'According to Robert Lafore, the transition to OOP brings several powerful new paradigms:\n\n1. Objects: The basic run-time entities in an object-oriented system. They may represent a person, a place, a bank account, or any item that the program must handle.\n\n2. Classes: A class is a blueprint or prototype from which objects are created. It defines the data and behaviors that the objects instantiated from it will possess. If a class is "Car", an object is "My specific blue Honda Civic".\n\n3. Inheritance: The process by which one class acquires the properties and behaviors of another. This supports the concept of hierarchical classification and heavily promotes reusability. For instance, a `Manager` class can inherit from a general `Employee` class.\n\n4. Reusability: Once a class is written and debugged, it can be distributed to other programmers for use in their own programs. This is similar to hardware components being reused in different electronic devices.\n\n5. Polymorphism: From Greek meaning "many forms". It allows operators or functions to behave differently depending on the context. For instance, the `+` operator adds numbers, but can be "overloaded" to concatenate strings or add matrices.',
      examples: [
        {
